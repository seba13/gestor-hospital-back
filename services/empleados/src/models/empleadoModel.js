...
export const getEspecialidadesMedicas = async () => {
  const [rows] = await promise.query(
    'select id_especialidad as id, especialidad as data from especialidad',
  );
  return rows;
};
...
export const getEspecialidadesPorId = async id => {
  const [rows] = await promise.query(
    `select id_especialidad as id, especialidad as data from especialidad where id_especialidad='${id}'`,
  );
  return rows;
};
